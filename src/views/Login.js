...
export default function Login() {
  ...
  const { currentUser, validateInput, login, loginComplete, loginAnonymous, loginAnonymousComplete, addPasskey, addPasskeyComplete, application, getApplication, socialSignup, socialLogin, logout } = useAuth()
  const [error, setError] = useState("")
  const [loading, setLoading] = useState(false)

  const navigate = useNavigate()
  const [requireAddPasskey, setRequireAddPasskey] = useState(false)
  ...
  const handleResetPasskey = async () => {
    try {
      ...
      if (token === undefined) {
        setError("Please enter reset passkey token")
        return
      }

      let data = { 'access-token': token };
      console.log("handleResetPasskey data ", data);

      let result = await addPasskey(data);
      if (result.error) {
        setError(result.error.message)
        return;
      }
      ...
      setRequireAddPasskey(false)

      setError("Your passkey has been reset successfully. Please login again.")
              ...
              <Card.Body>
                ...
                {requireAddPasskey ?
                  <Form>
                    ...
                    <div className="w-100 text-center mt-2 mb-4">
                      <h6 className="mt-20 text-primary">Your account has been requested for reseting passkey. Please enter your reset passkey token below.</h6>
                    </div>
                    <Form.Group id="token">
                      <Form.Label className="gray-text">Enter Passkey Token</Form.Label>
                  
                      <Form.Control as="textarea" rows={3} value={token} name="token" required className="small-text" autocorrect="off" autocapitalize="none" onChange={onChangeToken} />
                    </Form.Group>
                    ...
                    <div className="w-100 text-center">
                      ...
                      {loading ? <Button variant="primary button-radius" disabled className="w-100 mt-3">
                        ...
                        <Button className="w-50 mt-3 button-radius" onClick={handleResetPasskey}>
                          Submit
                        </Button>
                      }
